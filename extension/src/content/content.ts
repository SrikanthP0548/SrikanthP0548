...
const textEl = panel.querySelector<HTMLTextAreaElement>('#logix-annotation-text')!;
const typeEl = panel.querySelector<HTMLSelectElement>('#logix-annotation-type')!;
const saveEl = panel.querySelector<HTMLButtonElement>('#logix-annotation-save')!;
const cancelEl = panel.querySelector<HTMLButtonElement>('#logix-annotation-cancel')!;

let currentSessionId: string | null = null;
let recordingStartedAt = 0;

function showRecordingBadge(show: boolean): void {
  badge.style.display = show ? 'block' : 'none';
}

function showPanel(show: boolean): void {
  panel.style.display = show ? 'block' : 'none';
  if (show) textEl.focus();
}
...
void chrome.runtime.sendMessage({ type: Msg.POPUP_GET_STATUS, sessionId: null, payload: {} }).then((resp: any) => {
